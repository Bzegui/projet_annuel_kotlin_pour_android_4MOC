...
class GetFiltredRecipesUC @Inject constructor(private val repository: RecipeRepository){
    operator fun invoke(filter: String): Flow<Resource<List<Recipe>>> = flow{
        try {
            emit(Resource.Loading())
            val recipes = repository.getFiltredRecipes(filter)
